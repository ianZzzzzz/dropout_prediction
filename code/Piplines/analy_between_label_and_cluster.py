...
def Analy(
    ...
    Args:
        drop_list (list): [description]
        nondrop_list (list): [description]

    Returns:
        dict: ['scene':[coverage gap ,avg gap]]
    """    
    def count_scene(log_:list)->dict:
        """[
            1 统计场景出现的频次
...
label_path = 'prediction_log\\7777777.csv'
...
analy_result = Analy(drop_list=list_droped_series,nondrop_list=list_nondrop_series)

feat = []
for key,value in analy_result.items():
    feat.append([int(key),value[0],value[1]])
import numpy as np
np_feat = np.array(feat)
np_feat = np_feat[ np_feat[:,1].argsort()]
