...
chunk_size = int(10000) # enable only when train_OR_NOT = True
def load(
    log_path: str,
    return_mode: str,
    encoding_='utf-8',
...
dict_train_label = list_to_dict(
    load(log_path='prediction_log\\train_truth.csv',
...
list_train_data = json.load(open(
    'json_file\\into_model\\list_train_static_info_dataset.json','r'))
list_train_label = json.load(open(
    'json_file\\into_model\\list_train_label.json','r'))
list_e_id_in_trainDataset_ordered = list(dict_train_label.keys())

dict_user_find_course = {}
dict_course_find_user = {}

for e_id in list_e_id_in_trainDataset_ordered:
    e_id = str(e_id)
    u_id = dict_enroll_find_user[e_id]
    c_id = dict_enroll_find_course[e_id]
...
dict_u_and_c_find_label = make_dict_course_and_user_find_enroll(
    dict_enroll_label = dict_train_label,
...
for e_id in list_e_id_in_trainDataset_ordered:
    ...
    train_data[row].append(c_amount)
    train_data[row].append(c_d_rate)
    train_data[row].append(u_amount)
    train_data[row].append(u_d_rate)
...
if len(data)==len(list_train_data):
    print('data len correct!')
else:
    print('data length unmatch!!!!!')

for row in range(len(list_train_data)):
    #ori_data = list_train_data[row]
    new_info = data[row]

    # 不用extend 怕指针出错
    for item in new_info:
        list_train_data[row].append(item)

json.dump(
    list_train_data,
    open('list_train_static_info_dataset_2.0.json','w'))
